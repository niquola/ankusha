...
(defn get-datasource [db-name]
  (let [database-url DB-URL
    ...
    {:idle-timeout       1000
     ...
     :data-source.url
     (str "jdbc:postgresql://" (:lcl/host master) ":" (get-in master [:lcl/postgres :pg/port])
          "/" db-name "?user=" (:usr/name user) "&password=" (:usr/password user) "&stringtype=unspecified")}))
